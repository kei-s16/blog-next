  ...
  return (
    <>
      ...
      <div className="flex flex-row content-center justify-center">
          <div className="basis-1/3 rounded-lg flex items-center justify-center">{prev && <Link href={`/categories/${encodeURIComponent(category)}?page=${prev}`}>previous</Link>}</div>
          <div className="basis-1/3 rounded-lg flex items-center justify-center"><Link href="/">◆</Link></div>
          <div className="basis-1/3 rounded-lg flex items-center justify-center">{next && <Link href={`/categories/${encodeURIComponent(category)}?page=${next}`}>next</Link>}</div>
      </div>
    </>
  );
};
